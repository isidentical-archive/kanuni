import time
import arcade
from arcade import color as Color, key as Key
...
CONFIG['window']['width'] = (CONFIG['tiles']['width'] + sep) * CONFIG['tiles']['col'] + sep + pad
CONFIG['window']['height'] = (CONFIG['tiles']['height'] + sep) * CONFIG['tiles']['row'] + sep + pad
...
class Player:
    ...
    def obtain_cell(self):
        for _ in range(len(self.cells)):
            ...
            cell = Cell(**{through: getattr(cell, through) + 1, opp: getattr(cell, opp)})
...
class Controller(arcade.Window):
    ...
    def draw_info(self):
        ...
        arcade.draw_text(f"Feeding Type: {self.player.features.feeding}", 250, 0, Color.BLACK, 16)
        arcade.draw_text(f"Food Modifier: {self.player.features.food}", 500, 0, Color.BLACK, 16)
        
        arcade.draw_text(f"Collect {self.player.required_points} DNA points and press G to evolve.", 150, 50, Color.BLACK, 16)
        arcade.draw_text(f"Generation: {self.player.level}", 290, 800, Color.BLACK, 16)
        arcade.draw_text(f"Speed: {self.player.features.speed}", 490, 800, Color.BLACK, 16)
    
        if self.event:
            arcade.draw_text(f"!!!!!!!!!!! - {self.event} - !!!!!!!!!!!!!!!!!!", 100, 830, Color.RED, 16)
    ...
    def update_player(self, utype, action, *args):
        ...
        if randrange(0, 250) == 66 and len(self.player.cells) > 1:
            self.player.loses += 1
            self.event = f"Oh no! One of your cells' died. Total {self.player.loses} cells died."
